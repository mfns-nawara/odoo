...
class MrpProduction(models.Model):
    ...
    def _compute_delay_alert_date(self):
        for production in self:
            late_moves = production.move_raw_ids.filtered(lambda m: m.delay_alert_date).sorted(lambda m: m.delay_alert_date)
            production.delay_alert_date = late_moves[-1].delay_alert_date if late_moves else False

    def _compute_json_popover(self):
        for production in self:
            production.json_popover = json.dumps({
                'popoverTemplate': 'mrp.ProductionPopover',
                'model': 'mrp.production',
                'delay_alert_date': production.delay_alert_date.strftime('%d/%m/%Y %H:%M:%S') if production.delay_alert_date else False,
                'late_elements': [{
                        'id': late_move.id,
                        'name': late_move.display_name,
                    } for late_move in production.move_raw_ids.filtered(lambda m: m.delay_alert_date).move_orig_ids.production_id
                ]
            })
