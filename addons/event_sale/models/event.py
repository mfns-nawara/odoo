...
class Event(models.Model):
    ...
    @api.depends('event_ticket_ids.sale_date_availability')
    def _compute_event_registrations_open(self):
        non_open_events = self.filtered(lambda event: not any(event.event_ticket_ids.mapped('sale_date_availability')))
...
class EventTicket(models.Model):
    ...
    sale_date_availability = fields.Boolean(string='Is Available', compute='_compute_sale_date_availability')
    ...
    def _compute_sale_date_availability(self):
        for ticket in self:
            current_date = fields.Date.context_today(ticket.with_context(tz=ticket.event_id.date_tz))
            if not ticket.product_id.active:
                ticket.sale_date_availability = False
                ...
                ticket.sale_date_availability = True
...
class EventRegistration(models.Model):
    ...
    def _prepare_attendee_values(self, registration):
        ...
        if line_id and line_id.event_ticket_id.sale_date_availability:
            att_data.update({
