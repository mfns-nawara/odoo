...
class ReSequenceWizard(models.TransientModel):
    ...
    first_move_id = fields.Many2one('account.move', domain="[('journal_id', '=', journal_id), ('posted_before', '=', 'True')]")
    end_date = fields.Date(compute="_compute_end_date", readonly=False, help="Date (inclusive) to which the numbers are resequenced. If not set, all Journal Entries up to the end of the period are resequenced.")
    first_name = fields.Char(compute="_compute_first_name", readonly=False, string="First New Sequence")
    ...
    @api.depends('journal_id', 'first_move_id')
    def _compute_end_date(self):
        self.end_date = False
        for record in self.filtered(lambda r: r.first_move_id):
            if record.journal_id.sequence_number_reset == 'year':
                record.end_date = get_fiscal_year(record.first_move_id.date)[1]
            elif record.journal_id.sequence_number_reset == 'month':
                record.end_date = get_month(record.first_move_id.date)[1]
    ...
    @api.depends('journal_id', 'first_move_id', 'end_date')
    def _compute_move_ids(self):
        for record in self:
            domain = [('journal_id', '=', record.journal_id.id)]
            domain += ['|', ('date', '>', record.first_move_id.date), '&', ('date', '=', record.first_move_id.date), ('name', '>=', record.first_move_id.name)]
    ...
    def _compute_preview_moves(self):
        for record in self:
            new_values = json.loads(record.new_values).values()
            changeLines = []
            in_elipsis = 0
            previous_line = None
    ...
    def _compute_new_values(self):
        ...
        for record in self.filtered('first_name'):
            ...
            if record.journal_id.sequence_number_reset == 'year':
                ...
                format = '{prefix1}%(year)s{prefix2}%(seq)0{len}d'.format(
                ...
                format = '{prefix1}%(year)s{prefix2}%(month)s{prefix3}%(seq)0{len}d'.format(
                    prefix1=sequence.group('prefix1'),
                    prefix2=sequence.group('prefix2'),
            ...
            for j, grouped_list in enumerate(sorted.values()):
                ...
                for move, new_name in zip(grouped_list.sorted(lambda m: m.date), new_name_list):
    ...
    def resequence(self):
        ...
        for move_id in self.move_ids:
            if self.ordering == 'keep':
                move_id.name = self.new_values[move_id.id]['new_by_name']
            else:
                move_id.name = self.new_values[move_id.id]['new_by_date']
            move_id.date = self.new_values[move_id.id]['state']
