...
class AccountReconcileModel(models.Model):
    ...
    def _prepare_reconciliation(self, st_line, aml_ids=[], partner=None):
        ...
        to_create, open_balance_vals, existing_lines = st_line._prepare_reconciliation(lines_vals_list)
            ...
            SELECT
                ...
                (
                    ...
                    AND
                        ...
                        && regexp_split_to_array(TRIM(REGEXP_REPLACE(st_line.payment_ref, '[^0-9|^\s]', '', 'g')), '\s+')
                )
                OR
            ...
            WHERE st_line.id IN %s
                ...
                (
                    ...
                    (
                        line_partner.partner_id = 0
                        AND
                        TRIM(REGEXP_REPLACE(st_line.payment_ref, '[^0-9|^\s]', '', 'g')) != ''
                        AND
                        (
                            (
                                aml.name IS NOT NULL
