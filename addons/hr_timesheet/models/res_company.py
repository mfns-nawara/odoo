...
class ResCompany(models.Model):
    ...
    def _default_project_time_mode_id(self):
        ...
        if not uom:
            uom = self.env['uom.uom'].search([('category_id', '=', self.env.ref('uom.uom_categ_wtime').id), ('uom_type', '=', 'reference')], limit=1)
        if not uom:
            uom = self.env['uom.uom'].search([('category_id', '=', self.env.ref('uom.uom_categ_wtime').id)], limit=1)
        return uom
