...
class MailThread(models.AbstractModel):
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        if message.get('References') and routes:
            ...
            self.env['mail.notification'].set_opened(mail_message_ids=message_ids)
            self.env['mail.notification'].set_replied(mail_message_ids=message_ids)
    ...
    def _routing_handle_bounce(self, email_message, message_dict):
        ...
        if bounced_msg_id:
            self.env['mail.notification'].set_bounced(mail_message_ids=[bounced_msg_id])
        if bounced_email:
            three_months_ago = fields.Datetime.to_string(datetime.datetime.now() - datetime.timedelta(weeks=13))
            stats = self.env['mail.notification'].search(['&', ('bounced', '>', three_months_ago), ('email', '=ilike', bounced_email)]).mapped('bounced')
