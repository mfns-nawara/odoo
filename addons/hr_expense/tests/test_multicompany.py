...
class TestExpenseMultiCompany(TestExpenseCommon):
    ...
    def test_expense_multicompany_company_propagation(self):
        ...
        print(self.env['ir.property'].get('property_account_expense_categ_id', 'product.category'))
        ...
        expense.with_context(allowed_company_ids=[self.company_a.id, self.env.company.id], company_id=self.company_a.id).action_submit_sheet()
        self.assertEquals(expense.state, 'submit', 'Expense is not in Reported state')
        self.assertEqual(expense.company_id.id, self.env.company.id)

        expense.with_context(allowed_company_ids=[self.company_a.id, self.env.company.id], company_id=self.company_a.id).approve_expense_sheets()
        self.assertEquals(expense.state, 'approve', 'Expense is not in Approved state')

        expense.with_context(allowed_company_ids=[self.company_a.id, self.env.company.id], company_id=self.company_a.id).action_sheet_move_create()
        ...
        #The company on the payement should be the same as the one on the expense, even if we are in a another company
        WizardRegister = self.env["hr.expense.sheet.register.payment.wizard"].with_context(
            active_model=expense._name, active_id=expense.id, active_ids=expense.ids, allowed_company_ids=[self.company_a.id, self.env.company.id], company_id=self.company_a.id
        )
