...
class Meeting(models.Model):
    ...
    stop = fields.Datetime('Stop', required=True, tracking=True, help="Stop date of an event, without time for full days events")
    ...
    duration = fields.Float('Duration', compute='_compute_dates', inverse='_inverse_duration')
    ...
    recurrence_id = fields.Many2one('calendar.recurrence.rule', index=True)
    ...
    def _apply_recurrence_values(self, values, future=True):
        ...
        for event in self:
            if not event.recurrence_id:
                ...
                event.recurrence_id.write(dict(values))
                to_update |= event.recurrence_id
    ...
    def _write_recurrence_shift(self, time_values, future=True):
        """
        Apply time changes to events in the recurrence and update the rrule accordingly.

        :param future: time changes are applied to future events only if True.
                       Changes are applied to all events in the recurrence otherwise.
        ...
        if future is not True and previous_duration != self.duration:
            recurrence._write_events({'duration': self.duration})
    ...
    def write(self, values):
        detached_events = self.env['calendar.event']
        recurrence_update = values.pop('recurrence_update', None)
        ...
        if recurrence_update in ('all_events', 'future_events') and len(self) == 1 and self.recurrence_id:

            if values.get('recurrency') is False:
                detached_events |= self._break_recurrence(future=recurrence_update == 'future_events')

            update_start = self.start if recurrence_update == 'future_events' else None
            time_values = {field: values.pop(field) for field in self.env['calendar.event']._get_time_fields() if field in values}
            detached_events |= self._write_recurrence_shift(time_values, future=bool(update_start))
        ...
        if values.get('start'):
            ...
            (current_attendees & previous_attendees)._send_mail_to_attendees('calendar.calendar_template_meeting_changedate')
    ...
    def create(self, values):
        ...
        detached_events = meeting._apply_recurrence_values({field: values.pop(field) for field in self._get_recurrent_fields() if field in values})
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        def my_events(events):
            ...
            """
            my = [event for event in events if event.get('user_id', [])[:1] == self.env.uid]
            others = [event for event in events if event.get('user_id', [])[:1] != self.env.uid]
            return my, others
