...
class RecurrenceRule(models.Model):
    ...
    first_event_id = fields.Many2one('calendar.event', compute='_compute_first_event')
    ...
    def _compute_first_event(self):
        self.calendar_event_ids.read(['start'])  # prefetch
        for recurrence in self:
            recurrence.first_event_id = recurrence.calendar_event_ids.sorted('start')[:1]
    ...
    def _reconcile_events(self, ranges):
        ...
        events_to_keep_ids = (event.id for event in self.calendar_event_ids if event._range() in ranges)
        events_to_keep = self.env['calendar.event'].browse(events_to_keep_ids)

        existing_ranges = set(event._range() for event in events_to_keep)
        ranges_to_create = (event_range for event_range in ranges if event_range not in existing_ranges)
        return events_to_keep, ranges_to_create
    ...
    def _apply_recurrence(self):
        ...
        for recurrence in self:
            event = recurrence.base_calendar_event_id or recurrence.calendar_event_ids.sorted('start')[:1]
            duration = event.stop - event.start
            starts = recurrence._get_occurrences(event.start)
            ranges = ((start, start + duration) for start in starts)
        ...
        detached_events = self.calendar_event_ids - keep
        detached_events.recurrence_id = False
    ...
    def _stop_at(self, event):
        ...
        """
        self.ensure_one()
        detached_events = self._get_events_from(event.start)
        detached_events.recurrence_id = False
        ...
        if event.allday:
            until = self._to_start_of_period(event.start_date)
        else:
            until_datetime = self._to_start_of_period(event.start)
    ...
    def _rrule_parse(self, rule_str, date_start):
        ...
        if rule._bynweekday:
            # LUL TODO clean this mess of indexes
    ...
    @api.model  # TODO rename this method
    def _to_start_of_period(self, dt):
        if self.rrule_type == 'daily':
            return dt
        elif self.rrule_type == 'weekly':
            return dt + relativedelta(weekday=MO(-1))  # TODO start of week from env lang
    ...
    def _get_occurrences(self, dtstart):
        ...
        """
        self.ensure_one()
        dtstart = self._to_start_of_period(dtstart)
