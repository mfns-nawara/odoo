...
class TestUpdateRecurrentEvents(TestRecurrentEvents):
    ...
    def test_shift_period_start(self):
        """ Slip then move first recurrence to the start or recurrence period """
        event = self.events[2]
        event.write({
            'recurrence_update': 'future_events',
            'start': event.start + relativedelta(days=4),
        ...
        self.assertEqual(self.recurrence.until, date(2019, 11, 3))
        event = self.events[1]  # still in the first recurrence
        event.write({  # move to start of period = Monday
            'recurrence_update': 'all_events',
            'start': event.start - relativedelta(days=1),
            'stop': event.stop - relativedelta(days=1),
        })
        self.assertEventDates(self.recurrence.calendar_event_ids, [
            (datetime(2019, 10, 21, 1, 0), datetime(2019, 10, 23, 18, 0)),
    ...
    def test_shift_all(self):
        ...
        self.assertEqual(self.recurrence.dtstart, datetime(2019, 10, 26, 1, 0))
        self.assertFalse(self.recurrence.tu)
        self.assertTrue(self.recurrence.sa)
    ...
    def test_update_recurrence_all(self):
        ...
        recurrence_events = self.recurrence.calendar_event_ids.sorted('start')

        self.assertEqual(recurrence_events[1], self.events[0], "Events on Tuesdays should not have changed")
        # self.assertEqual(recurrence_events, self.events[2], w"Events on Tuesdays should not have changed")
        self.assertEventDates(self.recurrence.calendar_event_ids, [
            (datetime(2019, 10, 21, 1, 0), datetime(2019, 10, 23, 18, 0)),
            (datetime(2019, 10, 22, 1, 0), datetime(2019, 10, 24, 18, 0)),
            (datetime(2019, 10, 28, 1, 0), datetime(2019, 10, 30, 18, 0)),
        ])
        self.assertEqual(self.recurrence.dtstart, datetime(2019, 10, 21, 1, 0))
        self.assertTrue(self.recurrence.tu)
        self.assertTrue(self.recurrence.mo)
...
class TestUpdateMultiDayWeeklyRecurrentEvents(TestRecurrentEvents):
    ...
    def test_shift_all_multiple_weekdays(self):
        ...
        self.assertFalse(self.recurrence.tu)
        self.assertTrue(self.recurrence.th)
        self.assertTrue(self.recurrence.fr)
        self.assertEqual(self.recurrence.count, 3)
        self.assertIn(self.events[1], self.recurrence.calendar_event_ids, "Friday Event should not have changed")
        self.assertEventDates(self.recurrence.calendar_event_ids, [
    ...
    def test_shift_all_multiple_weekdays_duration(self):
        ...
        self.assertEventDates(self.recurrence.calendar_event_ids, [
            ...
            (datetime(2019, 10, 25, 1, 0), datetime(2019, 10, 28, 18, 0)),
...
class TestUpdateMonthlyByDay(TestRecurrentEvents):
    ...
    def test_shift_all(self):
        ...
        # 2nd Thursdays
        self.assertEventDates(self.recurrence.calendar_event_ids, [
            (datetime(2019, 10, 10, 1, 0), datetime(2019, 10, 12, 18, 0)),
            (datetime(2019, 11, 14, 1, 0), datetime(2019, 11, 16, 18, 0)),
            (datetime(2019, 12, 12, 1, 0), datetime(2019, 12, 14, 18, 0)),
        ])
