...
from odoo.tools.misc import humanize_number, get_field_value_string
...
class LeadPlsDescriptionField(models.TransientModel):
    ...
    recordset_size = fields.Char('Recordset size', readonly=True)
...
class LeadPlsDescription(models.TransientModel):
    ...
    lead_probability = fields.Float(related='lead_id.automated_probability', readonly=True)
    lead_fields = fields.One2many('crm.lead.pls.description.field', 'lead_pls_description_id', readonly=True)

    sale_team_id = fields.Many2one('crm.team', readonly=True, related='lead_id.team_id')
    closed_leads_count = fields.Char(string='Number of leads of this team', readonly=True)

    win_probability = fields.Integer('Team winning rate (%)', readonly=True, help='Probability to win a lead')
    ...
    def default_get(self, fields):
        ...
        result.update({
            ...
            'sale_team_id': current_lead.team_id.id,
            'closed_leads_count': humanize_number(lead_info.get('team_total')),
            'win_probability': lead_info['p_won'] * 100,
            'lead_fields': [
                (0, 0, {'name': _('Tags') if field_name == 'tag_id' else self.env['crm.lead']._fields[field_name].string,
                        'value': get_field_value_string(self.env['crm.lead'], 'tag_ids' if field_name == 'tag_id' else field_name, field_value),
                        'recordset_size': humanize_number(won + lost),
